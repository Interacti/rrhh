...
class Banners extends CI_Controller {
    ...
    function __construct() {
        ...
        if (!$this->session->userdata('bo_isLogued')) {
            ...
            $this->load->model('categorias_temas_model','categorias');
    ...
    function Agregar() {
        ...
        $datos['categorias']=$this->categorias->getSubCategorias();
    ...
    function Guardar() {
        ...
        $this->form_validation->set_rules('url', 'url', 'trim|xss_clean|valid_url_format');
        $this->form_validation->set_rules('target', 'target', 'trim|xss_clean');
        $this->form_validation->set_rules('fecha_inicio', 'fecha_inicio', 'trim|xss_clean|max_length[10]');
        $this->form_validation->set_rules('fecha_termino', 'fecha_termino', 'trim|xss_clean|max_length[10]');
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            if (is_array($sbi) && count($sbi) > 0) {
                ...
                $inBanner = array(
                    ...
                    "bnr_categoria" => $this->input->post('categoria'),
    ...
    function Actualizar() {
        ...
        $this->form_validation->set_rules('fecha_inicio', 'fecha_inicio', 'trim|xss_clean|max_length[10]');
        $this->form_validation->set_rules('fecha_termino', 'fecha_termino', 'trim|xss_clean|max_length[10]');
        $this->form_validation->set_rules('posicion', 'posicion', 'integer');
        $this->form_validation->set_rules('estado', 'estado', 'required|integer');
        ...
        if ($this->form_validation->run() === FALSE) {
           ...
           /* if ($_FILES ['bnr_imagen_r'] ['name'] == "") {
            ...
            $inBanner = array(
                ...
                "bnr_url" => $this->input->post('url'),
                "bnr_target" => $this->input->post('target'),
                "bnr_inicio" => $this->input->post('fecha_inicio')=="0000-00-00" ? "2018-01-01" : $this->input->post('fecha_inicio'),
                "bnr_termino" => $this->input->post('fecha_termino')=="0000-00-00"? "2099-01-01" : $this->input->post('fecha_termino') ,
                "bnr_posicion" => $this->input->post('posicion'),
                "bnr_estado" => $this->input->post('estado'),
                "bnr_fecha" => date('Y-m-d h:i:s')
            );
