...
class Beneficios extends CI_Controller {
    ...
    function Index() {
        redirect('bo/beneficios/Listar');
    }

    function Listar() {
        $datos ['content'] = 'backend/beneficios/listar'; // llamada al content de este metodo
        $datos ['hide'] = false;
        $datos ['data'] = $this->beneficios->getRows();
        $datos ['seccion'] = array(
            'beneficios',
            '/bo/beneficios',
            'Listado',
            '/bo/beneficios/Listar'
        );
        $datos ['titulo'] = "Administrador de beneficios / Listado";
        $this->load->view('backend/layout', $datos); // llamada a la vista general
    }

    function Agregar() {
        $datos ['content'] = 'backend/beneficios/agregar'; // llamada al content de este metodo
        $datos ['hide'] = false;
        $datos ['titulo'] = "Administrador de beneficios / Agregar beneficios";
        $con['conditions'] = array(
            'category' =>0
        );
        $datos['categorias']=$this->beneficios->getRows($con);
        $datos ['seccion'] = array(
            'beneficios',
            '/bo/beneficios',
            'Agregar beneficios',
            '/bo/beneficios/agregar'
        );
        $this->load->view('backend/layout', $datos); // llamada a la vista general
    }
    ...
    function Editar($id) {
        ...
        $datos ['modulo'] = 'beneficios';
        $datos ['accion'] = 'actualizar';
        $datos ['rs'] = $this->beneficios->getRows($con);
        $datos ['content'] = 'backend/beneficios/editar'; // llamada al content de este metodo
        $datos ['hide'] = false;
        $cat['conditions'] = array(
            'category' =>0
        );
        $datos['categorias']=$this->beneficios->getRows($cat);
        $datos ['titulo'] = "Administrador de beneficios / Editar beneficios";
        $datos ['seccion'] = array(
            'beneficios',
            'bo/beneficios',
            'Editar Noticia',
    ...
    function Guardar() {
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            redirect(BASE_URL . 'bo/beneficios/Agregar');
        } else {

            $imagen_mini ['upload_path'] = './assets/frontend/img/beneficios/';
            ...
            if (is_array($sbi) && count($sbi) > 0) {
                ...
                $config_prin ['upload_path'] = './assets/frontend/img/beneficios/';
                ...
                if (is_array($sbd) && count($sbd) > 0) {
                    ...
                    $config_attach ['upload_path'] = './assets/frontend/pdf/beneficios/';
                    ...
                    if (!$this->beneficios->insert($in)) {
                        ...
                        redirect(BASE_URL . 'bo/beneficios');
                    }
                } else {
    ...
    function Actualizar() {
        ...
        if ($this->form_validation->run() === FALSE) {
            $this->session->set_flashdata('errors', validation_errors());
            $this->session->set_flashdata('formdata', $this->input->post());
            redirect(BASE_URL . 'bo/beneficios/Editar/' . $this->input->post('id'));
        } else {
            ...
            if (!$this->beneficios->update($in, array("id" => $this->input->post('id')))) {
    ...
    function Activar($id = 0) {
        ...
        $update = $this->beneficios->update($cmsData, array('id' => $id));
